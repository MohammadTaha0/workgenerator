...
if ($hire == "hired") {
    $msg = 'Congrats, You Hired By ' . $Comp_name . ' Company as a ' . '"' . $job_name . '"';
    $delete = mysqli_query($con, "DELETE FROM `notification` WHERE `send_id`='$app_id' AND `rec_id`='$rec_id' AND `sen_role`='0' AND `rec_role`='1'");
    if ($delete) {
        $insert_noti = mysqli_query($con, "INSERT INTO `notification`(`send_id`,`rec_id`,`sen_role`,`rec_role`,`msg`,`date`) vALUES('$app_id','$rec_id','0','1','$msg',current_timestamp())");
    }
} elseif ($hire == "req") {
    $msg = 'Oops, You Hired By ' . $Comp_name . ' Company as a ' . '"' . $job_name . '" but later he was change his decisions';
